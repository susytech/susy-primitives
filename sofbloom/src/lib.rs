...
extern crate sophon_types_serialize;
...
impl Serialize for Bloom {
	fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error> where S: Serializer {
		...
		sophon_types_serialize::serialize(&mut slice, &self.0, serializer)
	}
}
...
impl<'de> Deserialize<'de> for Bloom {
	fn deserialize<D>(deserializer: D) -> Result<Self, D::Error> where D: Deserializer<'de> {
		...
		sophon_types_serialize::deserialize_check_len(deserializer, sophon_types_serialize::ExpectedLen::Exact(&mut bytes))?;
		Ok(Bloom(bytes))
	}
}
